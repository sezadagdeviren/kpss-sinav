...
app.get('/api/categories', async (req, res) => {
  try {
    const { sinav_turu } = req.query;
    let query = 'SELECT DISTINCT kategori FROM questions';
    const params = [];
    if (sinav_turu) {
      query += ' WHERE LOWER(sinav_turu) = LOWER(?)';
      params.push(sinav_turu);
    }
...
app.get('/api/years/:category', async (req, res) => {
  try {
    const { category } = req.params;
    const { sinav_turu } = req.query;
    const sTuru = sinav_turu || 'Lisans';
    const [rows]: any = await pool.query(
      'SELECT DISTINCT yil FROM questions WHERE LOWER(kategori) = LOWER(?) AND LOWER(sinav_turu) = LOWER(?) ORDER BY yil DESC',
      [category, sTuru]
    );
    res.json(rows.map((r: any) => r.yil.toString()));
  } catch (err) {
    res.status(500).json({ error: 'Yıllar yüklenemedi' });
  }
});
...
app.get('/api/stats/:category?/:year?', async (req, res) => {
  try {
    ...
    let query = `
      ...
      FROM questions q
      LEFT JOIN user_activity ua ON q.id = ua.question_id
      WHERE LOWER(q.sinav_turu) = LOWER(?)
    `;
    const params = [sTuru];
...
app.post('/api/reset', async (req, res) => {
  try {
    const { category, kategori, year, yil, sinav_turu } = req.body;
    const finalCategory = category || kategori;
    const finalYear = year || yil;
    const finalSinavTuru = sinav_turu || 'Lisans';
    await pool.query(`
      UPDATE user_activity ua
      JOIN questions q ON q.id = ua.question_id
      SET ua.status = 'empty', ua.user_choice = NULL
      WHERE LOWER(q.kategori) = LOWER(?) AND q.yil = ? AND LOWER(q.sinav_turu) = LOWER(?)
    `, [finalCategory, finalYear, finalSinavTuru]);
    
    await pool.query('DELETE FROM user_exam_summaries WHERE LOWER(kategori) = LOWER(?) AND yil = ? AND LOWER(sinav_turu) = LOWER(?)', [finalCategory, finalYear, finalSinavTuru]);
    res.json({ success: true });
  } catch (err) {
...
app.post('/api/exam-summary', async (req, res) => {
  try {
    const { category, kategori, year, yil, sinav_turu, last_time, last_correct, last_wrong, last_empty } = req.body;
    const finalCategory = category || kategori;
    const finalYear = year || yil;
    const finalSinavTuru = sinav_turu || 'Lisans';
    ...
    await pool.query(`
      INSERT INTO user_exam_summaries (kategori, yil, sinav_turu, last_time, last_correct, last_wrong, last_empty)
      VALUES (?, ?, ?, ?, ?, ?, ?)
      ON DUPLICATE KEY UPDATE 
    ...
    `, [finalCategory, finalYear, finalSinavTuru, last_time, last_correct, last_wrong, last_empty]);
    res.json({ success: true });
  } catch (err) {
...
app.get('/api/exam-summaries/:category', async (req, res) => {
  try {
    ...
    const finalSinavTuru = sinav_turu || 'Lisans';
    const [rows] = await pool.query(
      'SELECT * FROM user_exam_summaries WHERE LOWER(kategori) = LOWER(?) AND LOWER(sinav_turu) = LOWER(?)',
      [category, finalSinavTuru]
    );
    res.json(rows);
  } catch (err) {
...
app.get('/api/review/:type/:category?', async (req, res) => {
  try {
    ...
    const { sinav_turu } = req.query;
    const sTuru = sinav_turu || 'Lisans';
    let query = "SELECT q.*, ua.status, ua.is_favorite, ua.user_choice FROM questions q LEFT JOIN user_activity ua ON q.id = ua.question_id WHERE LOWER(q.sinav_turu) = LOWER(?)";
    const params = [sTuru];
...
app.get('/api/mistakes-by-year/:category', async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(`
      ...
      JOIN questions q ON q.id = ua.question_id 
      WHERE LOWER(q.kategori) = LOWER(?) AND ua.is_in_mistake_pool = 1 AND LOWER(q.sinav_turu) = LOWER(?)
      GROUP BY q.yil
    `, [category, sTuru]);
    res.json(rows);
  } catch (err) {
...
app.get('/api/favorites-by-year/:category', async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(`
      ...
      FROM user_activity ua 
      JOIN questions q ON q.id = ua.question_id 
      WHERE LOWER(q.kategori) = LOWER(?) AND ua.is_favorite = 1 AND LOWER(q.sinav_turu) = LOWER(?)
      GROUP BY q.yil
    `, [category, sTuru]);
    res.json(rows);
  } catch (err) {
